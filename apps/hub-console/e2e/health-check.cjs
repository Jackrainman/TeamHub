 ...
 *
 * 装置前置（SETUP-WIZARD 刀②，setup-wizard.md §8）：两态启动后，server 在 config.json **不存在**时进
 * setup 模式、只渲染首启动向导（正常 10 页不挂）。故本脚本两种用法各需对应的起服前置：
 *   1. 常规 10 页体检（默认 HEALTH_EXPECT=normal）：起服前**预置合法 config.json**（demo+anonymous =
 *      等价旧默认行为），server 进正常模式再打。检测到向导 = 前置没做好，直接 fail-fast 报明。
 *   2. 向导渲染用例（HEALTH_EXPECT=wizard）：起服前**不要 config.json**（或指向不存在路径），server 进
 *      setup 模式；本脚本断言全屏向导 + 两张大卡渲染、无报错 / 白屏。
 * server 生命周期与 config 预置由起服方负责（本脚本只连 BASE，沿脚本一贯的"外部起服"模型）。
 *
 * 用法：
 *   npm run health-check                                   # 默认打 http://localhost:4177，断言正常 10 页
 *   TEAMHUB_BASE=http://host:4177 npm run health-check
 *   HEALTH_EXPECT=wizard npm run health-check              # 断言首启动向导渲染（起服须无 config.json）
...
(async () => {
  ...
  if (EXPECT === 'wizard') {
    ...
    const problems = [];
    if (!wizardPresent) problems.push('未渲染首启动向导（.setup-wizard 缺失）——起服时是否误留了 config.json？');
  ...
  // ── 常规 10 页体检：起服须已预置 config.json（正常模式）。检测到向导 = 前置没做好，fail-fast ──────
  if (wizardPresent) {
    await browser.close();
    console.error('[health] ✗ FAIL — server 处于 setup 模式（渲染的是首启动向导）。');
    console.error('  常规体检前须预置合法 config.json（demo+anonymous），让 server 进正常模式（setup-wizard.md §8）。');
