...
| PM-BOARD-DESIGN | pending (P0, frontier) | design | **项目计划表**（D-041 定调，原“项管看板”；高强度 + 最省力）= 进度透明 + 对接找谁。**任务为核心 · 全员可见 · 不按人**：依赖图 + 任务状态 + 缺口 + 分工（谁负责），**卡住必带原因**（在等谁/等什么，禁“光秃秃天数+人名”）· **无甘特**（预设确定工期/顺序/硬截止皆无，违 G4）。复用 `Task/Dependency/Need/Group/Member`（状态机/owner 已齐），补 due date / 优先级两字段 + 依赖录入（并入原 `GOV-DEP-INTAKE`：布置任务顺手连依赖 + AI 预填）。判断交人、AI 不派活·不排名；**人治视图（按人天数/甘特/空闲检测/排班）封存**（D-041 决策 7③）|
