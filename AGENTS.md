...
- **组织结构（任务分配单元，D-072 定稿）**：分配任务**只有四个组**——电控 / 视觉 / 机械 / 电路，**设置页可增减**。**「程序组」不作领任务单元**（删该身份）；「程序 = 电控 + 视觉」仅保留为**汇报 / 过载向上合并视角**，不直接领任务、不在排班里作独立单元。**总联调 = 所有组各到至少一人**（不挂单一组）。fixtures 遗留（`grp-program` 仍持 `t-r1/r2-integration` + `m-progA/B`）下一轮调和：成员归电控/视觉、总联调改全组各一人、`grp-program` 去领任务身份。`grp-*` 组树本身已正确（机械/电路顶层·程序顶层挂电控/视觉）。
