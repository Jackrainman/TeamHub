...
frontier:                                # PRESENCE-RECONCILE-LOCK 需定稿；其余仍卡外部基建 / 用户排期
  - IA-REFACTOR                          # 【阶段1已落地 D-075；阶段2-4 IMPLEMENTING D-076·分支 ia-phase2-4(云端 master a4033b8 已干净并入)·未回 merge master】信息架构重构：10 平铺页按数据域重组(计划见 docs/design/sched-date-relay-robot-redesign.md B 节)。**①机器人队页=已做**(FleetPage 组合不重写/导航10→9/改状态画布即时反映 prefix失效/画布定高，三包 verify 全绿+**WSL 真机 Playwright 全 PASS** buildId ce8d99a，截图 docs/screenshots/wsl-fleet-*)。**②项目页(看板+依赖图)→③知识页(kb+档案)+表单一致→④导航分组(主操作区/洞察区可折叠/设置) = 2026-06-20 用户拍板 D-076 一轮收尾，4 层 workflow 实现中**。拍板覆盖项：**gaps=C**(缺人方向留独立顶级洞察项·非并入项目页 Tab，性质同总览=只读仪表盘→洞察区与总览并排，导航 9→8→7)；**Phase4 仅导航分组无工作台**(落地页保留总览)；表单一致并入 P3。待本机 verify + WSL 真机验收 → 标完成。
  - IA-FORM-CONSISTENCY                   # 【2026-06-20 拍板 D-076·并入 IA-REFACTOR Phase 3 实现中】图纸提交 与 机器人队 create 对「赛季/机器人维度」控件不一致。锁定：①赛季统一下拉 seasonOptions(now)±2 年 + 「其它/手填」兜底；②第三项保留两套语义(robotCode 通用=图纸适配 vs robotTarget 共享=占位)、契约枚举不动、只统一控件风格+文案；③控件够像可抽 <SeasonSelect>否则不强求。spec=ia-refactor-next-prompts.md PROMPT 2。
