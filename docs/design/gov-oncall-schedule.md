...
排班按**小组**、视觉上**按大组分组**（汇报合并的同一层级），让"程序在场 / 电控随叫 / 视觉去学"在同一个大组下并存而不打架：
...
│     · 总联调  🟢 在场     持有 R1（最后一关）      │
│     · 电控    🟡 随叫     底盘调试上游、仍在攻坚    │
│     · 视觉    ⚪ 今晚不用来  被「底盘调试」卡 · 资料→│
│  ▸ 电路大组                                       │
│     · 电路    🟡 随叫     新版电路板，要和电控一起看│
│  ▸ 机械大组   （今晚链上无活，沉默）              │
│  ⚠ 程序大组在关键链 → 别再往这组加新任务（私下）  │
└───────────────────────────────────────────────────┘
```
...
- `recommendations[]`：`derivePresenceSchedule` 派生结果，**先按 `reportingGroupId`（大组）分组、组内按小组列**；`mode` 分色：
...
- **汇报滚动到大组**：`reportingGroupId` 同样是组键（无人维度）。给老师 / 项目级汇报按**大组**聚合（A2"老师只看项目级"）；排班的小组细节（电控随叫 / 视觉去学）停在控制台内部，不进对外汇报——汇报层只说"程序大组今晚在联调"。
