...
| KB-IMPORT-FOLLOWUP | **部分 done (D-051)** / 剩 nit ①② pending (P2, 非阻塞) | code | **D-051 独立二次对抗审计(wf_74dee37d-59b)硬化已收口**：除 D-050 的 nit ③④ 外，二审另抓 3 条真实正确性/§10 缺陷并修——**fileNameToSlug 撞 slug→静默丢档**(哈希后缀单射)、**toIsoDateTime 不查日历→整档 failed**(Date.UTC 反查)、**EISDIR 崩整批**(withFileTypes+isFile + readFile try/catch)；顺手收 SKIP_FILES 大小写(④)/extractSection `**`残留(③)/死代码/isCli 正则。+9 测、verify:all 绿。**剩余 nit（非阻塞、二审确认召回无损）**：① IMPORT_FORCE=1 重导无去重(仅磁盘膨胀) ② 汇总文档 rootCause `；`串接(全文留 rawInput)。详 decisions.md D-051 |
...
| AUDIT-FIXES-2026-06-14 | 部署前必修 7 条 done (D-059) + 2nd 批 M8/L4/M13/M21 done (D-065) / 长尾 pending | code | **代码审计修复批次** —— 详单见 `docs/planning/code-audit-2026-06-14.md`（15-agent 对抗审计，confirmed 42：High 5 / Med 16 / Low 12 / Nit 3）。**部署前必修 7 条 done (D-059)**：H1 环守卫 / H2 写链重置 / H3 写鉴权+限流+bodyLimit / H4 status clamp / M6 剥 confirmedBy / H5+M11 compose / M9 单调序号。**2nd 批 done (D-065，2026-06-15 审计后 server 硬化)**：M8（invoke safeParse+400）/ L4（closeout 201）/ M13（构造器补全克隆 8 数组）/ M21（console 写侧测试）+ 预写部署代码（SystemStatus.mode enum / trustProxy / DEMO_SEED 空板）。**长尾 pending**（后续 console 批 / 部署批）：M14/M15/M16 前端 a11y、M17/M18/M19 KB 字段 .max/.min、M20 workspace 工具、L1/L5-L12、N1-N3、KB schema 双声明、bridge/members（用户拍暂留·真实部署前必处理）、ownerId/ownerLabel 设计张力（AGENTS §5 对账） |
