...
current_task: PM-BOARD-DESIGN  # KB-CORE done(D-044)后顺推 PM。已落 PM-U1：GovStore.createTask 写实现 + POST /api/tasks（承接 base 收口刀录入簇，hub-server verify 31 测）。**已触 §8 设计闸**：createDependency/createNeed 的 confirmedBy=ActorRef 与 D-042「confirmedBy=timestamp 非 memberId 守 I0」冲突=planning↔代码冲突+涉 I0 宪法核 → 待用户拍板再续录入簇；console PM 板写侧 UX 亦待定向
