...
frontier:                                # PRESENCE-RECONCILE-LOCK 需定稿；其余仍卡外部基建 / 用户排期
  # ✅ IA-REFACTOR 完成(2026-06-21 终态 D-077)：①机器人队页 D-075；②项目页(看板+依赖图视图切换)③知识库(纯相似检索)④图纸档案(独立)⑤导航全摊平 8 项。**D-076 首版(知识页合并图纸+洞察折叠分组)被用户验收推翻→revert 整个 Phase2-4 从干净 D-075 重做(996df7d revert / 9c4cc5d 重做)**。终态扁平导航固定序：总览/项目/知识库/图纸档案/库存/机器人队/缺人方向/设置。本机三包 verify 全绿+i18n 双侧 467 键平衡；组合不重写/零契约改/I0 净/赛季 SeasonSelect 表单一致保留。**WSL2 真机 Playwright 待补**(测试机 ssh 超时离线，脚本+bundle 已传，上线即跑→截图 docs/screenshots/wsl-ia-fix-*)；**待 push origin/master**(local master 领先 origin 3 commit：996df7d/9c4cc5d/docs)。详 decisions.md D-077(D-076 全文已归档)。
