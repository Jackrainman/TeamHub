...
## 0. 当前定位（协作中枢：CASE + 交流中心 + 数据库，D-037 回中）

### 当前判定
Teamhub = 机器人战队的 **协作中枢 / Team Hub**：融合 CASE 工具 + 团队交流中心 + 战队数据库（D-026 立"进度治理"魂、**D-037 回中去监视味**，覆盖 D-024 的"信息路由器 / 监控 broker"定位）：给学长**减负** / 给学弟**指引** / **项目同步进度表**；进度卡住、当事人羞于开口时靠"卡点在依赖图上自动可见"的**结构**替他说出来，提前暴露隐含依赖让所有人动起来。**核心不变式 I0（D-037）：人键只回本人当帮助、第三方只见结构键；公平靠给被卡的人正名、不靠抓摸鱼。** 详见 D-026/D-037、`AGENTS.md §1`、`docs/design/team-hub-concept.md`。

> **D-039 第一轮落地（2026-06-13）**：第一轮 **AI 退出治理**——②规则/协调层（魂）降为**人读说明视图**（大三/学长人工判断协调，AI 不判定·派活·排名）；其 AI 派生整簇（D-032~D-035）挂起、想法不丢。本轮产品 = **战队内部协作工具三支柱**（① 知识库 ② 项管看板 ③ 库存-BOM，全 P0），演进留地基。下方四层 / 当前主线为治理期骨架，治理判断部分已随 D-039 挂起、留作复活蓝图；详见 `decisions.md` D-039 + `backlog.md`。
...
| ②规则/协调层（魂） | 卡点 / 过载 / 缺口暴露(结构键给协调者) · 被卡正名 · 给本人的自指帮助(silence/腾出手只回本人, I0) | 新增 |
...
**路线 A**：系统关系库是真相，飞书只做汇报 / 通知 / check-in 出口，不双写。已建 Hub 壳子（hub-server / hub-contracts / hub-console / Compose）作为触点/集成 + 展示底座保留，规则/协调域是新增核心。

当前主线：

1. 设计宪法三层重构（C/G/A）+ 文档体系 reframe（本轮）。
2. 治理数据真相层数据模型（项目/赛季、成员+角色+资历、组织树、任务+依赖 DAG、Need、共享物理资源/占用窗口 D-029）。
3. 规则/治理层（卡点 / 过载 / 沉默 / 升级判定 + 按依赖位置差异化在场派生 D-029）。
4. 展示/汇报层（动态最短任务周期图"结构+状态"版、"谁该在场"一屏、给老师的自动汇报）。
5. 触点/集成层：飞书 check-in / 通知 / 汇报派生（路线 A），复用已建 Lark 三包与 Hub 壳子。

已拍板：架构走法（D-028 治理为主轴，2026-06-11）、提醒可见范围 / 送达模型（D-026 后续，2026-06-10）、差异化在场排班=共享资源门控派生（D-029，2026-06-11）、**定位回中 + 人键自指化（D-037，2026-06-12：CASE+交流中心+数据库、核心不变式 I0、silence 纯自指、机会导向协调视图）**。
...
- **协作专属原则 G1-G5**：用共享真相替代凭感觉 / 系统是大脑飞书是脸 / 暴露缺口 / 无硬截止只轻提醒 / 对资历弱者兜底。
- **反监视四原则 A1-A4**：暴露需求不暴露"人慢了" / 人键提醒只回本人(不上报管理者) · 老师只看项目级 / 面向个人=纯给予 / 无 deadline 只轻推（人-silence 永不升级）。
