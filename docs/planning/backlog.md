...
> 三根全 P0（**D-040 破冰序 `base → kb → pm → inv`**，**D-042 定基调**：base 补收口刀、KB 拆 CORE/LARK、PM 删 Member.status+结构键、INV 留着排最后对话记账防死、Hermes 最后接）。frontier 顺序：**base 两刀 done → KB-CORE done（D-044，2026-06-14）→ #1 PM**（KB-LARK/INV/Hermes 后置）。设计北极星：比死掉的表格更省事 ｜ 用着就更新（派生优先）｜ AI 只当仓管·转译不下判断 ｜ 人在环 ｜ 小作坊轻量。共享底座 = `HUB-SERVER-GOV-SCAFFOLD`（持久层 + real CRUD 路由），任一根先做都先过它。详见 `docs/design/three-pillar-feasibility.md`。
