...
| HUB-SERVER-GOV-SCAFFOLD | 首刀 done / 剩余 pending (frontier#1, **D-039 共享底座**) | code | **D-039 重定位为三支柱共享底座**：持久层（现全 mock）+ real CRUD 路由骨架（知识库/项管/库存 的 `GET/POST /api/...`；现 real `GET /api/dep-graph` 等 404）+ `now=server clock` 注入——所有真实数据流的物理出入口，**做一次三根受益**；store 先 seed fixtures 让 real 路由不再 404。原"GovernanceStore/派生地基"语义随治理挂起，先服务 CRUD 而非派生。**D-040 首任务收敛**=只注册 `GET /api/dep-graph`（`MockStore`+`Clock`+`toDepGraphView` 派生）解 real 模式 404，写入簇/presence/drizzle 后置；DoD/边界/接口契约见 `docs/design/three-pillar-reqdesign.md` §2。**首刀已落地（2026-06-13，commit）**：`GET /api/dep-graph` + `GovStore`/`InMemoryGovStore`(seed)/`Clock` 注入，hub-server+hub-contracts verify:all 全过。**D-042 下一刀（frontier#1）= 收口刀**：`GovStore` 加写方法白名单签名 + `BuildHubServerOptions` 加 `kbStore?/invStore?` 扩展点（接口先行≈30 行，1 atomic-task），化解"四次重建底座"违 C3；补强=`GovernanceSnapshot` 已含 knowledgeNodes/taskKnowledgeTags、KB 复用同快照不必扩 interface、真正要扩的只有 INV PartStock；base DoD 增持久化切换合约 + SqliteGovStore stub 证同接口可落地。写入簇实现/presence/drizzle 仍 pending |
...
> 三根全 P0（**D-040 破冰序 `base → kb → pm → inv`**，**D-042 定基调**：base 补收口刀、KB 拆 CORE/LARK、PM 删 Member.status+结构键、INV 留着排最后对话记账防死、Hermes 最后接）。frontier 顺序：**#1 base 收口刀 → #2 KB-CORE → #3 PM**（KB-LARK/INV/Hermes 后置）。设计北极星：比死掉的表格更省事 ｜ 用着就更新（派生优先）｜ AI 只当仓管·转译不下判断 ｜ 人在环 ｜ 小作坊轻量。共享底座 = `HUB-SERVER-GOV-SCAFFOLD`（持久层 + real CRUD 路由），任一根先做都先过它。详见 `docs/design/three-pillar-feasibility.md`。

| 任务 | 状态 | type | 内容 |
|------|------|------|------|
| KB-CORE-DESIGN | pending (P0, frontier#2, **D-042 拆**) | design | **战队知识库·核心（零飞书、可立即开工、四根里最快交付）**：移植 Probe_Flash `IssueCard→InvestigationRecord→ErrorEntry→ArchiveDocument` 闭环到 `hub-contracts/src/kb.ts`（**保留 normalizedSummary/relatedFiles/relatedCommits 三字段**否则 `buildCloseoutFromIssue` 报 TS2339；import 头按 verbatimModuleSyntax 改 `.js`/单引号/`import type`）+ 移植 `rankSimilarIssues` 纯函数段（~95 行非整 323 行）+ `GET /api/kb/similar`（症状→top-N 相似 bug）+ `buildCloseoutFromIssue` + `POST /api/kb/closeout` 结案派生 `KnowledgeNode` 标注。复用 `growth.ts KnowledgeNode/TaskKnowledgeTag`（树从标注长出、不预设本体 C3）。北极星：用着就沉淀，不做"事后填总结"。DoD 第一条=先落 kb.ts schema 草图 + 产"触发表{用户动作→派生路径→写目标}+死表基线+findability 可测搜索路径"。**A4 护栏措辞**：AI 只列候选相似项/检查单条目、不断言同因、由人选用 |
...
| PM-BOARD-DESIGN | pending (P0, frontier#3) | design | **项目计划表**（D-041 定调，原“项管看板”；高强度 + 最省力）= 进度透明 + 对接找谁。**任务为核心 · 全员可见 · 不按人**：依赖图 + 任务状态 + 缺口 + 分工（谁负责），**卡住必带原因 = 结构键**（在等哪个上游任务/组/Need，点开才见组、对接才见人；禁“光秃秃天数+人名”）· **无甘特**（违 G4）。复用 `Task/Dependency/Need/Group/Member`（状态机/owner 已齐）+ 依赖录入（并入原 `GOV-DEP-INTAKE`：布置任务顺手连依赖 + AI 预填）。**D-042 收口**：删 `Member.status`/freeIdle 任何展示通道（取最新版 D-041，`DepGraphPage` 的 `freeIdleCount` 复用须去掉）；`confirmedBy`=timestamp 非 memberId（守 I0）；blockedBy 走 Dependency 边由 `toDepGraphView` 派生、不在 Task 上另存（守 G2）；**`dueDate` 本轮不引入**（甘特暂缓+违 G4），priority 改 `criticalChain` 派生；`TaskSchema` 必填 projectId/rawSummary/robotTarget/intrinsicComplexity/statusSource 须 server 默认或表单补齐。判断交人、AI 不派活·不排名；人治视图（按人天数/甘特/空闲检测/排班）封存（D-041 7③）|
