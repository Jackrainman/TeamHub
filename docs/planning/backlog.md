...
| PM-BOARD-DESIGN | **partial：后端 done (D-045, 2026-06-14)，console UI 后置** | design | **项目计划表**（D-041 定调）。**后端录入簇+读视图已落地**（§6.B 连续构建 + 2-opus 对抗核实 ship/mustFix=0）：`createTask`/`createDependency`(clamp active)/`createNeed`(clamp open+claimedByMemberId=null A2 反派单) 写实现 + `POST /api/tasks`·`/api/dependencies`·`/api/needs` + `GET /api/tasks` 读视图。**I0 读写边界拍板（用户 Q1）**：confirmedBy=ActorRef 内部凭证，永不经读视图暴露/排名（探针实证守住）；blockedBy 走 Dependency 边由 `toDepGraphView` 派生(G2)；dueDate 不引入(G4)；ownerId 仅「谁负责」(C2 安全堆)。设计+落地=`docs/design/pm-board.md`。**剩余（frontier#1 下一轮，用户 Q2 定 console UI 下一轮）**：console PM 看板 UI（@xyflow 列表+看板双视图 + mutation 表单 + 冷启动空板引导）/ 依赖录入 AI 预填 / criticalChain→priority 派生 / 真实 status 派生上游。人治视图（按人天数/甘特/空闲检测）封存（D-041 7③）|
