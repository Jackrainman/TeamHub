...
- 更早条目（**D-031** 概念调研 9 agent + frontier 重排（数据生命线命门比录入更深：hub-server 治理路由空 + Member.status 无派生 + freeIdle 误判 + 3 处空壳）、**GOV-C4-FIX**(`17316cc`) 修两处 C4 破口（relatedKnowledgeFor/computeCriticalSet）+ 锚定测、**D-030** 文档瘦身 −46%、2026-06-11 planning-sync 录入立项 + frontier 重排、GOV-SCHED-MODEL/**D-029** 差异化在场排班 + `derivePresenceSchedule`、GOV-DATAMODEL-VIZ-ARCHPATH/**D-028** 治理为主轴 + 依赖链归因视图、GOV-REMIND-AXIS-DECIDE/**D-027** 成长轴 + 提醒模型、PF-V03-CLEANUP / HUB-COMPOSE-SMOKE 等）见 `git log` 与 `decisions.md`——已裁剪到 5 条（AGENTS §6）。
