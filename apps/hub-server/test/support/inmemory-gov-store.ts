 ...
 * mock-gov-store-base.ts）上按域叠 mixin——pm-core（PmCoreStore 全方法）/ schedule（ScheduleStore
 * 全 12 条）。artifact 域已随 ARCH-UNIFY A4 摘出（test/support/inmemory-artifact-store.ts）。方法体逐字搬迁、零行为变化；`implements GovStore` 的编译期校验由
 * 消费点承担（server.ts `const store: GovStore = … new InMemoryGovStore(…)` / 旧生产 Store 全方法委托 inner）。
 *
