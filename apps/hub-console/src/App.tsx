...
export function App() {
  ...
  const setupQuery = useSetupState(apiClient);
...
function ConsoleApp({ apiClient, settings }: { apiClient: HubApiClient; settings: AppSettings }) {
  ...
  const sessionQuery = useSession(apiClient);
  ...
  const overviewQuery = useOverview(apiClient, SOURCE, identityCacheKey(identitySession));
        ...
        <div>
          ...
          <h1>
            ...
            {activePage?.beta ? (
              <span
                className={
                  activePage.beta === 'public-beta' ? 'beta-badge beta-badge--public' : 'beta-badge'
                }
              >
