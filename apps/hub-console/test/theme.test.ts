...
describe('主题：normalizeTheme（localStorage 存储值收敛）', () => {
  test("合法值原样返回：'warm' / 'classic' / 'dark' / 'tech'", () => {
  ...
  test('null / 空 / 未知值 → 默认 tech（D1：遥测台为旗舰默认；已存偏好者读 localStorage 不被覆盖）', () => {
    expect(normalizeTheme(null)).toBe('tech');
    expect(normalizeTheme('')).toBe('tech');
    expect(normalizeTheme('TECH')).toBe('tech'); // 大小写敏感，非精确匹配即 fallback 到默认
    expect(normalizeTheme('WARM')).toBe('tech');
  });
});
