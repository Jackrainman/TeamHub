...
current_task: "2026-07-03 五件套：① daily-plan-presets 实现（D-082，设计稿已拍板）② 阶段二前置——console 侧装配接线消费 TenantConfig（现 console-pages.tsx 全量静态注册未消费）+ i18n/vocabulary-overrides.ts 接线 + 补非默认 TenantConfig 装配路径测试（此前从未跑过）③ L6 飞书网关入站白名单+限流快赢（code-audit 42 条仅剩这条未修）+ KbSearchPage segClass 快赢 ④ 全部完成后 WSL 全量验证（三包 verify:all + Playwright health-check）⑤ 前端 UX 美化轮（表单重点）。下一步顺序 = ①②③ 可并行推进 → ④ 收口 → ⑤。历史（2026-06-19 INV-BOM-CORE+PRESENCE-IMPL、07-01~02 模块化阶段一 等）见 completed-log。"
repo_sync: "2026-07-03：origin/master=c84f4a9（07-03 FF 合并 feat/plugin-core 六个模块化 commit：6fc32fb→c84f4a9，VERSION=0.9.7）。rainman WSL2 上次同步到 feat/plugin-core 同一 HEAD（07-02 验证全绿：三包 verify:all 432 测 + 迁移脚本 scripts/migrate-robottarget.mjs 真 gov.json 副本零丢失 + Playwright health-check 8 页 0 错等价 master + 真实未迁移 gov.json 新 schema 加载成功）；本轮（daily-plan-presets/阶段二前置/L6）代码改完需重新 bundle 同步 WSL 验证。**部署前必做**：D-080 砍 `requires` 依赖类型 + FileGovStore fail-closed 校验后，旧 gov.json（含 `type:'requires'`）redeploy 直接起不来，须先跑 `scripts/migrate-robottarget.mjs` 或 reset `~/teamhub-data/gov.json`。 // 历史 2026-06-21：origin/master=9cd0b7a（D-078 图纸文件链路 + 表单修缮 + 部署/版本号修复），三包 verify:all 全绿（164/194/44）+ 真机 API 冒烟全过（上传/下载/重传清兄弟/落盘持久化/415·401·404·400）。 // 历史 2026-06-19：origin/master 曾推进到无人值守实现轮（5cb84cb：INV-BOM-CORE + PRESENCE-IMPL 安全增量 + WSL 验收截图 + lark bin 修，4 commit 已 push）。push 凭证走共享库 ~/ruolin_huang/.gh-credentials，按 D-064 默认 commit+push（push 前 git fetch 防分叉）。rainman WSL2 真机已同步到 736f0c3 并起服 4177 跑通 Playwright 验收（loopback，TEAMHUB_INV_DATA_FILE=~/teamhub-data/inventory.json）；注意 WSL 在 ssh 会话关闭后会 reap 所有进程（含 tmux），常驻部署须保持连接或 enable-linger，验收已在单会话内完成。bundle 传 100.78.202.84 实测 ssh BatchMode 可达、字节比对无截断。详见 memory teamhub-remote-access。"
