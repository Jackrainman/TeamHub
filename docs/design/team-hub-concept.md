...
> **D-037**：本层产出统一为 `GovernanceCue`（见 `docs/archive/suspended-specs/gov-cue-layer.md`，挂起）。人键信号（沉默 / 腾出手 / 被卡去学）**只回本人当帮助**；结构信号（缺口 / 过载 / Need 升级 / 卡点链）才给协调者，且为事 / 组键。
...
> **已拍定 2 — 提醒可见范围 / 送达模型（D-026 后续，2026-06-10；D-037 收窄）**：私聊本人、升级的是事不是人、AI 起草不发送/建议不判定/检索不评价。**D-037 收窄**：人键提醒（silence / 腾出手）**只回本人 + AI 建议、不上报队长**（旧"提醒=队长轮询自动化 / 问责上移"废除）；只有结构信号（缺口 / 过载 / Need 升级）才路由协调者。见 `decisions.md` D-037 + `docs/archive/decisions-archive.md`「D-026 后续」（原文已归档）。
...
> **已拍定 4 — 定位回中 + 人键自指化（D-037，2026-06-12）**：定位从"制度化进度治理系统"回中为 **CASE 工具 + 团队交流中心 + 战队数据库**（给学长减负 / 给学弟指引 / 项目同步进度表）。**核心不变式 I0**：人键输出只回本人当帮助、第三方只见结构键。silence 收为纯自指（只回本人 + AI 建议），问责上移废除，"没派活+被卡+没主动接"= **机会导向协调视图**（管理者只看工作分配：待派活 + 过载组 + 组级前瞻余力，不点个人）。见 `docs/archive/suspended-specs/{gov-cue-layer,gov-role-visibility}.md`（挂起）。
