...
current_task: KB-CORE-DESIGN  # §6.B continuous-build 连续构建中（Claude Code）。已落：U1 kb.ts schema 链+kbScenarioFixture / U2 rankSimilarIssues 纯函数（跨赛季召回，hub-contracts verify 37 测）。在途：U3 buildCloseout+deriveKnowledgeNode / U4 KbStore 收窄 / U5 GET·kb/similar / U6 POST·kb/closeout / U7 设计文档+ADR。每单元各自 verify+单独 commit+push，不 STOP
