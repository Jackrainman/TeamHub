...
describe('inv-preview（刀⑪）：i18n 双语齐全', () => {
  ...
  test('向导步序号锚点：库存 = ⑥/(6)，完成 = ⑧/(8)（fleet ⑤ 与 done 之间插入，season ④ 见 season-step.test.ts）', () => {
    expect(translations.zh['gate.step.inventory']).toContain('⑥');
    expect(translations.en['gate.step.inventory']).toContain('(6)');
    expect(translations.zh['gate.done.title']).toContain('⑧');
    expect(translations.en['gate.done.title']).toContain('(8)');
  });
});
