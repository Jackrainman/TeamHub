...
## 4. Modes
- `governance_design`（**当前生效**，D-026）：把 Teamhub 落为制度化进度治理系统——按四层架构推进数据真相层实体（项目/赛季 · 成员角色资历 · 组织树 · 任务依赖 · Need）、规则/治理层、展示/汇报层。每个代码任务先有接口契约或 schema；adapter / 真实飞书 / 服务器写入 mock-first，真实凭证与写入需用户线下配置或审批。**待拍板开放项**：架构走法（治理为主轴 vs 平行模块）、提醒可见范围/送达模型——深设计文档先搭骨架留待定，不阻塞宪法/定位/planning 推进。
