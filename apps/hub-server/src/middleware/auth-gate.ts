...
/**
 * 读闸门 + 首登 PIN 闸（AUTH-GATE 公网加固，D-083 身份模式收紧）。
 *
 * 背景：公网暴露后「匿名可读一切」不再成立。身份模式下——
 *  ① **读闸**：未登录（无会话）一律 401，只放行白名单（登录/会话自身、setup 初始化、名册读取——
 *     登录表单与 BootstrapGate 都要先看到名册；名册只回 MemberPublic 剥 pinHash 视图）。
 *  ② **首登 PIN 闸**：会话成员无 pinHash（首次登录）时，一切业务请求 403 PIN_SETUP_REQUIRED，
 *     只放行 PUT 本人 pin / session 端点——先设 PIN 再进应用，服务端兜底不依赖前端自觉。
 *
 ...
 *  后台不开「加后缀就能进」的口子：/api/setup/* 只放行 state（初始化标记+设置单例，低敏）与
 *  super-admin bootstrap（路由自带「已有持旗成员 → 409」一次性门）；config/graduate 等敏感写
 *  未登录 401、登录后还须路由层 superAdmin。 */
function isPreLoginAllowed(method: string, path: string): boolean {
  if (path === '/api/session') return true; // GET 探模式 / POST 登录 / DELETE 登出
  if (method === 'GET' && path === '/api/setup/state') return true; // console 启动闸探测
  if (method === 'POST' && path === '/api/setup/super-admin') return true; // 零管理员初始化（一次性门）
  if (method === 'GET' && path === '/api/members') return true; // 登录表单成员下拉 + BootstrapGate 判定
...
export function registerAuthGate(app: FastifyInstance, opts: AuthGateOptions): void {
  const { identityMode, memberHasPin, trustProxy } = opts;
  ...
  app.addHook('onRequest', async (request, reply) => {
    ...
    if (!hasPin && !isPinSetupAllowed(request.method, path, request.identity.memberId)) {
