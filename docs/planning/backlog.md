...
> 战队机器人垂直场景内核向其他协作场景（游戏工作室/软件开发等）复用。两阶段：阶段一（已完成）把机器人单体模块化成 CASE base + 机器人层、master 不动；阶段二从合并后 master 切 game-studio/software-dev 垂直包 worktree，前置 = console 装配接线。设计北极星 `docs/design/modularization-feasibility.md`（已执行）/`docs/design/core-plugin-architecture.md`（更宽愿景，PROPOSAL 待对齐）。
...
| DAILY-PLAN-PRESETS | **本轮在建 (D-082)** | code | 每车 defaultPreset 一键铺表格→微调→进泳道图，收口日期锚定空屏债；设计稿 `docs/design/daily-plan-presets.md` 已拍板，实现口径见 D-082（复用优先/PATCH preset 端点/批量建 session 原子端点/空状态路由） |
| PHASE2-CONSOLE-ASSEMBLY | **本轮在建** | code | 阶段二前置：`console-pages.tsx` 全量静态注册未消费 TenantConfig + `i18n/vocabulary-overrides.ts` 未接线，均待接线；补非默认 TenantConfig 装配路径测试（此前从未跑过） |
| MODULARIZATION-PHASE2 | pending（待 PHASE2-CONSOLE-ASSEMBLY 完成） | code | 从合并后 master 切 game-studio/software-dev 等垂直包 worktree |
