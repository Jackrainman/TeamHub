...
- 报账域是 D-090 首个模块模板试点，目标见 `docs/domains/reimburse.md`。红线：票据解析在浏览器本地（服务器无 multipart 入口）；凭证原件留档按 D-094 只对「本人→财务」读者链可见，永不进聚合；条目人键只回本人+超管，批次聚合无按人明细；跨域入库改由 application service + 事务编排，route 不直调两个 repository。
