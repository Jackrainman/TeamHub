...
frontier:                                # PRESENCE-RECONCILE-LOCK 需定稿；其余仍卡外部基建 / 用户排期
  # ✅ IA-REFACTOR 全四阶段完成(2026-06-20)：①机器人队页 D-075(分支 ia-phase1-fleet)；②项目页③知识页+表单一致④导航分组 D-076(分支 ia-phase2-4)。导航 10→7(项目=看板+依赖图视图切换 / 知识库=相似检索+图纸档案 Tab / 主操作区·洞察区可折叠·设置 分组)，gaps=C 留独立洞察项与总览并排，落地页保留总览。三包 verify 全绿 + WSL2 真机 Playwright 10/10 PASS(buildId d0f858c，截图 docs/screenshots/wsl-ia-phase2-4-*)。**未回 merge master**：ia-phase2-4 已并入云端 a4033b8，回并到 master 现为干净快进，待用户拍板触发。组合不重写/零契约改/I0 净。详 decisions.md D-076。
