...
```yaml
mode: collab_design
stage: "2026-06-19 **frontier 见底·转演示准备**。三支柱读写 + 差异化排班 S1/S2/S3 + 图纸档案 v2 + 集成模型三分 + 治理快照落盘全落；剩 INV 第三支柱 / KB-LARK / 正式部署 / DEPGRAPH-AI-AUTODRAW 全卡外部门或用户排期。权威 = decisions.md D-061~D-072 + docs/archive/completed-log.md（逐条完成记录）+ git log。"
...
current_task: null  # frontier 见底（2026-06-19）：A 档连续自主批全清、B 档 SCHED-MEMBER 提前做掉；剩 INV 第三支柱(卡 HUB-HERMES-ADAPTER)/KB-LARK(卡 LARK-BIN-PROBE)/正式部署(卡审批)/DEPGRAPH-AI-AUTODRAW(卡 Hermes) 全卡外部门 → 转演示准备。逐条完成记录见 completed-log。
repo_sync: "2026-06-19：origin/master 推进到 052285d（D-069 P1 批，已全 push）。push 凭证走共享库 ~/ruolin_huang/.gh-credentials，按 D-064 默认 commit+push。rainman WSL2 真机仍停 D-068(230c38e)，落后约 13 commit；演示前需 git bundle 过 SSH 同步 230c38e..052285d + 三包 verify:all + 4177 起服走查新增「在场排班」页——bundle 传输到 100.78.202.84（用户自己电脑·非默认 SSH 端口），需用户在场确认可达后执行。详见 memory teamhub-remote-access。"
frontier:                                # 见底：可执行项全卡外部基建 / 用户排期
...
open_for_decision:                       # ARCH-PATH(D-028)/提醒(D-026后续→D-037)/资源(D-029)/idle三态+静默(D-032)/受众路由(D-033) 已拍；以下待用户线下细化
  ...
  - PRESENCE-VIZ-RESOURCE-MODEL          # 差异化排班表现形式重定向(三视图：依赖图+甘特项目进度+接力顺序链) + 资源两层模型(车位 RobotSlot/具体车 RobotInstance 生命周期+拆件血缘)+库存最小内核 = 设计稿立项(D-072，docs/design/presence-resource-redesign.md)；零代码·待用户定稿
post_pivot_registry:
