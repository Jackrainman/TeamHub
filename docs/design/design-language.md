...
| 批 | 内容 | 量 | 风险 | 状态 |
|---|---|---|---|---|
| B0 | UX 扫描 + 本规范 + 截图基线入库（docs-only） | S | 零 | ✅ 391014c |
| B1 | 文案统一（§7，只动 translations.ts） | S | 零 | ✅ 32c4278 v0.19.1 |
| B2 | 徽章归并（§3） | M- | 低 | ✅ 0a22eca v0.19.2 |
| B3 | 按钮体系（§2）——**全方案最大回归面，完成后停一拍出图给用户** | M | 中 | ✅ 6999319 v0.19.4 |
| U1 | 选中态修复（独立 commit：三主题侧栏 active 复活 + seg 统一，特异度战争根因） | S | 低 | ✅ 9e1b036 v0.19.5 |
| B4 | 卡片基类 + 空态收口（§4/§5，含 U5 幽灵格/U6 warm 边界顺带） | M | 中 | ⏸ 待用户看 B3 图放行 |
| B5 | 图标档 + kb- 前缀卫生（Field 吐 `.field` 族）+ CSS 治理（§6/§8，含 U4 滚动条顺带） | S-M | 低 | ⏸ 同上 |
