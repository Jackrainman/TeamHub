...
frontier:                                # PRESENCE-RECONCILE-LOCK 已定稿(D-079)→阶段② 待实现(提示词已交付)；其余仍卡外部基建 / 用户排期
  ...
  - PRESENCE-RECONCILE-LOCK               # ✅ 定稿完成(2026-06-21，D-079，路线 C + 7 决议全锁)：今晚=平日差异化三态 / 总联调日=全组各一人(convergenceScope='allLeafGroups' + grp-convergence 哨兵)。决议要点：sess 改名 sess-tonight-ec·invitedMemberIds=[]·总联调日加 R2·convergenceScope 进 console「全组」徽章·m-progA→grp-ec role=member·need-rtos→grp-ec。**阶段② 待实现**(AI 只写文档，实现提示词已交付用户去跑)：动 governance/fixtures/schedule/attribution + ~10 测试 + DAG 徽章 + AGENTS §1(勿碰 §7/scripts)；提交走 D-074 自动 bump(feat→minor)。实现真相 = presence-reconcile-lock.md §11 决议表 + §12 实现清单。
