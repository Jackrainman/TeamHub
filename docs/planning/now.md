...
current_task: null  # console 两支柱读视图 UI + 整体汉化 done(D-046)：KB 检索页(症状→相似历史，A4 note+reasons 原样、Mock 复用同一 rankSimilarIssues)+ PM 看板页(5 列 status 看板，卡片无 ownerId/完成量维度 C2)+ 整体汉化(enum/aria/语言自名全过 t()，zh/en 143:143 严格对称)。3-lens 对抗审计 wf_64a78d61-109 ship(I0/C2/A2/A4 clean)、hub-console verify:all(typecheck+7 测+build)绿、本地 Playwright 真机视觉验收 4 态(总览/KB 检索/PM 看板/EN)。**写侧 mutation 表单仍后置**。下一 frontier#1 = PM/KB 写侧录入表单(调 POST 路由)
frontier:                                # base 两刀 + KB-CORE + PM 后端 + console 读视图 UI/整体汉化 done → 剩 PM/KB 写侧 mutation 表单；KB-LARK(hardblock LARK-BIN-PROBE)/INV/Hermes 后置；治理派生 仍挂起
  - PM-KB-WRITE-FORMS                    # frontier#1 剩余 = console **写侧** mutation 表单：建任务/依赖/Need(调 POST /api/tasks·dependencies·needs)+ KB 结案录入(POST /api/kb/closeout)+ 冷启动空板引导。读视图(看板/检索)已落 D-046。**I0 护栏延续**：UI 永不显示 confirmedBy/谁快谁慢/完成量；ownerId 只显「谁负责」。后端事实源=docs/design/pm-board.md(§3 I0 读写边界/§4 触发表)+ kb-core.md
