...
current_task: null  # PM 后端录入簇+读视图 done(D-045)+2-opus 对抗核实 ship/mustFix=0(push 3bbf919)。用户 Q2=console UI 下一轮(用户定边界，连续轨在此告一段落)。下一 frontier#1 = PM console 看板 UI(写侧从零)
frontier:                                # base 两刀 + KB-CORE done + PM 后端 done → 剩 PM console UI；KB-LARK(hardblock LARK-BIN-PROBE)/INV/Hermes 后置；治理派生 仍挂起
  - PM-BOARD-DESIGN                      # frontier#1 剩余 = console PM 看板 UI（后端录入簇+读视图已落 D-045，用户 Q2 定 console UI 下一轮）。复用 hub-console @xyflow DAG 页模式：列表+看板双视图 + 任务/依赖/Need mutation 表单(调本轮 POST /api/tasks·dependencies·needs)+ 冷启动空板引导。卡住原因走 GET /api/dep-graph blockedByLabel(结构键)。**I0 护栏延续**：UI 永不显示 confirmedBy/谁快谁慢/完成量；ownerId 只显「谁负责」。后端事实源=docs/design/pm-board.md(已落 §3 I0 读写边界/§4 触发表)
