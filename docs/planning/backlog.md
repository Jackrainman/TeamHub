...
| TASK-POST-CLAIM | pending（**设计已锁 D-088**，2026-07-15；实现待排期，"先试试边做边改"——阈值全可调常量） | code | **挂单认领制**（口径=`docs/design/task-post-claim.md`）：Task 三 optional 字段（claimedAt/assignReason/本组搭档位）+ 认领/指派/转派路由（actor 注入沿 IDENTITY-LITE）+ 大任务判定纯函数（有下游边或挂门=大）+ console 挂单池/一键认领/搭档黄标/空态文案（"没把握就去问问学长"）/打回状态 + "看谁做过"事实卡搜索 |
| GATE-CHECKLIST-IOU | pending（**设计已锁 D-087**，2026-07-15；实现待排期，建议先行于 Hermes——纯本地零外部依赖） | code | **门检查单与欠条**（口径=`docs/design/gate-checklist-iou.md`）：GateChecklistItem schema（挂门或自选到期日/origin=template\|iou/豁免=大三+强制理由+留名）+ gate pass 校验拦截（检查项全非 pending 才可过门）+ deriveBaselineDrift 扩展（欠条到期红）+ console 门详情检查单卡+全局 30 秒"快记欠条"入口 + 模板 seed 留空等复盘导入 |
