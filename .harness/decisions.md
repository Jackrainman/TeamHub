...
## D-094 — 报销凭证红线拆分：解析留浏览器，原件改受控留档（REIMBURSE-EVIDENCE，2026-09-05 用户拍板）

- **原红线焊了两件事**：① 票据解析在浏览器本地运行（架构选择：服务器不装 PDF/OFD/ZIP 解析依赖、无 multipart 入口）；② 文件本体永不上传（隐私声明）。**①继续有效，②自本决定起作废**——两者过去被写在同一句里，导致任何一侧的改动都牵动另一侧，是本条要终结的混乱源。
- **②作废依据是读者链，不是「隐私不重要」**：报销原件的最终去向本来就是「本人 → 战队财务 → 学校财务/老师」，战队此前用腾讯填表系统即要求上传发票 PDF + 付款截图。故真实约束从来不是「文件不得进任何服务器」，而是「不得比既有流程更暴露」。
- **①不变**：OCR/解包/识别全部留在浏览器，识别结果仍需人工确认才成正式条目；`docs/design/software-architecture.md` 「文件解析」选型行不变。
- **②替换为访问不变式**：凭证附件是**条目级私有事实**——只对提交本人与承担财务职责的成员可读（沿用现有 authz 口，「财务」权限旗 2026-09-06 拍板不单独立，见下），永不进列表、聚合、统计或任何无鉴权静态路径，下载留操作者痕迹。I0 不受影响：附件回指本人，第三方只见结构键。
- **风险口径澄清**：域名/HTTPS 只解决传输窃听（Caddy 反代与 `TEAMHUB_COOKIE_SECURE` 已在先，见 D-092），**不解决附件的静态存**风险（磁盘、备份、未来某次读闸误配置）。故本决定的代价要由上面的访问不变式承担，不由 TLS 承担。
- **备份口径**：附件目录与 `teamhub.sqlite` 同进 `scripts/backup-teamhub-data.sh` 的备份/恢复范围；实际存放位置属部署动作，不在本域代码内拍板。
- **财务权限旗（2026-09-06 拍板）**：不单独立「财务」权限旗——「承担财务职责的成员」沿用现有超管/PM 旗判定，REIMBURSE-EVIDENCE-STORE 落地时按此实现。
- **诚实边界**：本决定只改规则，不改代码。**当前仍无任何上传端点**，前端「文件本体不用上传」文案与 `materials` checklist 布尔描述的是现状、不是承诺，在 REIMBURSE-EVIDENCE-STORE 落地前不得对外声称已可留档。
