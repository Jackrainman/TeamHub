...
current_task: "2026-07-11 **产品重定义 + 基准线设计双文档轮已落地**：①docs/design/product-redefine-2026-07.md（DECIDED，D-083）②docs/design/baseline-design.md（DESIGN-LOCKED v1：三版车节奏模板/验证门大二证据+大三验收/投资三维分类/版次可裁剪/周粒度红黄绿；证据字节走 artifact 卷不进 SQLite）+ core-plugin-architecture.md 归档 + backlog 防爆肝任务簇 + AGENTS 注记。**BASELINE-DESIGN 设计已锁，下一步 = BASELINE-CORE 实现 workflow**（交接提示词已给用户，可清上下文直接跑）；真实时间线用户赛后回填模板 v2。// 上轮 07-03 五件套终态 a64e7bc v0.10.3 详 completed-log；★本机有全套 node_modules+chromium 可本机验证；datalist 兼容性待真机确认。"
repo_sync: "2026-07-03：origin/master=c84f4a9（07-03 FF 合并 feat/plugin-core 六个模块化 commit：6fc32fb→c84f4a9，VERSION=0.9.7）。rainman WSL2 上次同步到 feat/plugin-core 同一 HEAD（07-02 验证全绿：三包 verify:all 432 测 + 迁移脚本 scripts/migrate-robottarget.mjs 真 gov.json 副本零丢失 + Playwright health-check 8 页 0 错等价 master + 真实未迁移 gov.json 新 schema 加载成功）；本轮 07-03 全部 commit 已经**本机全量验证**（WSL 跳过·用户裁定；本机 node24 首次装齐四包 node_modules + playwright chromium）：506 测全绿 + health-check 8 页 0 错 + 新端点冒烟全通。WSL 真机部署验证转为可选项。**部署前必做**：D-080 砍 `requires` 依赖类型 + FileGovStore fail-closed 校验后，旧 gov.json（含 `type:'requires'`）redeploy 直接起不来，须先跑 `scripts/migrate-robottarget.mjs` 或 reset `~/teamhub-data/gov.json`。 // 历史 2026-06-21：origin/master=9cd0b7a（D-078 图纸文件链路 + 表单修缮 + 部署/版本号修复），三包 verify:all 全绿（164/194/44）+ 真机 API 冒烟全过（上传/下载/重传清兄弟/落盘持久化/415·401·404·400）。 // 历史 2026-06-19：origin/master 曾推进到无人值守实现轮（5cb84cb：INV-BOM-CORE + PRESENCE-IMPL 安全增量 + WSL 验收截图 + lark bin 修，4 commit 已 push）。push 凭证走共享库 ~/ruolin_huang/.gh-credentials，按 D-064 默认 commit+push（push 前 git fetch 防分叉）。rainman WSL2 真机已同步到 736f0c3 并起服 4177 跑通 Playwright 验收（loopback，TEAMHUB_INV_DATA_FILE=~/teamhub-data/inventory.json）；注意 WSL 在 ssh 会话关闭后会 reap 所有进程（含 tmux），常驻部署须保持连接或 enable-linger，验收已在单会话内完成。bundle 传 100.78.202.84 实测 ssh BatchMode 可达、字节比对无截断。详见 memory teamhub-remote-access。"
frontier:                                # 本轮(07-11)=产品重定义文档轮完成；下一批按路线 v4（D-083）：基准线→轻身份→我的视图→store 拆分+SQLite
  ...
  - BASELINE-CORE                         # 刀①实现(下一 workflow)：Season 接线(现死脚手架)+baseline schema 落 zod+独立 baselineStore 两实现+落盘 baseline.json+路由(含过门 POST)+deriveBaselineDrift 纯函数+单测+总览页"基准线 vs 实际"+模板 v1 seed；口径=baseline-design.md，偏离回写
