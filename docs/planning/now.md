...
current_task: "2026-07-24 **公测补强刀①~④ 已全部落地**——刀④ PROGRAM-GROUP-ABSTRACT（本 commit，v0.32.0：方案②结构派生，「可选组=叶子组且非哨兵」由 deriveLeafGroups 单一源派生、零 Group schema 改动；写入口=importRoster 组名命中非叶子/哨兵组拒行进 failed（RosterImportRow 带行号指回原行，mock/file/sqlite 三 store 同刀）+ POST /api/tasks groupId 命中抽象组 400（未入表 id 维持宽松）；读出口=GET /api/groups 全量不动+补派生位 assignableGroupIds，PmCreatePanel 候选过滤到可选集合并修「显示组 id 而非中文名」UX bug（显示 name、提交映射回 id）；顺带补 D-072 前置缺口最小组管理=POST/PUT/DELETE /api/groups（叶子组可建/改名/删除，删除防孤儿 409，身份模式须持旗 403）+ 设置页「组」分区（非叶子/哨兵标注「汇报视角·不出现在选择」））；刀① `c23667c` v0.28.0、刀②b `b6281bc` v0.29.0、刀③ `0cc0ac4` v0.30.0、刀② `5941be1` v0.31.0。剩余排队=刀⑤ 运维动作（备份→重置已部署实例(4177)→用户冒烟新向导全流程）。真相 = docs/design/onboarding-pin-deadlock-2026-07-24.md。注：hub-contracts/package-lock.json 有他人 WIP 漂移（engines 字段），本批不动、不入 commit。"
repo_sync: "2026-07-24：公测补强刀④ PROGRAM-GROUP-ABSTRACT（本 commit，v0.31.0→0.32.0 minor；contracts 301/server 433/console 165 测全绿，三包 verify:all exit 0） + 刀② SETUP-WIZARD-ROSTER（`5941be1`，v0.31.0） + 刀③ ROSTER-CSV-3COL（`0cc0ac4`，v0.30.0） + 刀②b MEMBER-PM-FLAG（`b6281bc`，v0.29.0） + 刀① PIN-DEADLOCK-RECOVERY（`c23667c`，v0.28.0） + HARNESS-DIET/DOCS-SLIM（纯 harness/docs 不 bump）。更早账单（07-23 PIN-RESET+SETUP-WIZARD v0.27.0 / 07-16 文档轮+K6+K8+tag v0.25.0+BETA-READINESS 四刀 / 07-15 及以前）→ docs/archive/completed-log.md + git log。"
frontier:                                # 本轮(07-11~07-12)=路线 v4 五把刀全部落地收口；✅ 条目真相统一 → docs/archive/completed-log.md + git log；下一批见下方「后置」
  ...
  # 公测补强代办(2026-07-24 立项，用户实测部署命中初始化/PIN 死锁)：刀①~④ ✅已全部落地(2026-07-24，v0.28.0~v0.32.0——PIN-DEADLOCK-RECOVERY / MEMBER-PM-FLAG / ROSTER-CSV-3COL / SETUP-WIZARD-ROSTER / PROGRAM-GROUP-ABSTRACT 方案②结构派生)——剩刀⑤ 运维动作：备份→重置已部署实例(4177)→用户冒烟新向导全流程。方案+活体复现证据=docs/design/onboarding-pin-deadlock-2026-07-24.md
