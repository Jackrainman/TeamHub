...
npm run verify:architecture                     # 单 workspace/lock/version + 依赖方向 + 迁移基线
...
| 仓库/架构 | `npm run verify:architecture` + 对应包 `verify:all` exit 0 |
| hub 代码 | `npm run verify:architecture` + 对应包 `verify:all` exit 0 |
...
- 报账域是 D-090 首个模块模板试点，目标见 `docs/domains/reimburse.md`。红线：票据解析在浏览器本地（服务器不装解析依赖，multipart 入口只搬凭证字节、不读内容）；凭证原件留档按 D-094 只对「本人→财务」读者链可见，永不进列表/聚合（条目对象出 HTTP 前 `evidence` 剥空，清单只走条目级专属端点，字节只经鉴权下载）；条目人键只回本人+超管，批次聚合无按人明细；跨域入库改由 application service + 事务编排，route 不直调两个 repository。
...
- 写注释站在**读者**视角：一句话用人话说明这段代码做什么 / 为什么这么做（非显然意图、workaround、魔法数来源、顺序敏感），不记录编码时的思考过程、推导历史或迁移动账。
- 鼓励删冗余注释：指向已删除代码/文件的注释、与代码逐字重复的注释、描述的声明已迁走的悬空 JSDoc，发现即删。
- 代码不直白处缺注释就补一句；简短的设计意图 + docs 决策编号（D-xxx / I0 / C1-C5）注释是合法资产，不在清理范围。
