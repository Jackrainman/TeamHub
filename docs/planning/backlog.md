...
> 三根全 P0（**D-040 破冰序 `base → kb → pm → inv`**，**D-042 定基调**：base 补收口刀、KB 拆 CORE/LARK、PM 删 Member.status+结构键、INV 留着排最后对话记账防死、Hermes 最后接）。frontier 顺序：**base 两刀 done → KB-CORE done（D-044）→ PM done（后端 D-045 / 读视图 D-046 / 写侧表单 D-048，2026-06-14）→ #1 KB-IMPORT-PROBEFLASH**（KB-LARK/INV/Hermes 后置）。设计北极星：比死掉的表格更省事 ｜ 用着就更新（派生优先）｜ AI 只当仓管·转译不下判断 ｜ 人在环 ｜ 小作坊轻量。共享底座 = `HUB-SERVER-GOV-SCAFFOLD`（持久层 + real CRUD 路由），任一根先做都先过它。详见 `docs/design/three-pillar-feasibility.md`。
...
| KB-IMPORT-PROBEFLASH | pending (frontier#1, D-047/D-048 后续) | code | **ProbeFlash `.debug-archive` 一次性导入**（`/home/rainman/projects/Probe_Flash/.debug-archive`，7 md + frontmatter）：markdown(symptom/status + body)→`IssueCard`/`ErrorEntry` best-effort 解析器 → `POST /api/kb/closeout` 回灌检索语料。用户已定"后续本地不再产 archive、全留服务器"，故是**一次性导入非长期同步**。注意 errorCode 用真实/历史时戳而非 FixedClock。归 KB-CORE(D-044)/闭环(D-047)/写侧表单(D-048) 后的收尾 |
