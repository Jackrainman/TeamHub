...
- **组织结构（任务分配单元，D-072）**：分配任务**只有四个组**——电控 / 视觉 / 机械 / 电路，**设置页可增减**；**「程序组」是抽象大组 = 电控 + 视觉**之和，不直接领任务，只用于汇报/过载向上合并。收敛任务（如总联调）可挂大组（沿 D-029）；当前 fixtures 仍把总联调挂 `grp-program`，归属待调和（D-072 §6 #1）。`grp-*` 组树本身已正确（机械/电路顶层·程序顶层挂电控/视觉）。
