...
import { GOVERNANCE_SCENARIO_NOW, scheduleScenarioFixture } from '@teamhub/hub-contracts';
...
} from '../../src/modules/schedule/logic.js';
import type {
  RelayHandoffDraft,
  ...
  ResourceStatusPatch,
  ScheduleRepository,
} from '../../src/modules/schedule/repository.js';
import { createIdSequence, nextSequentialId, type IdSequence } from '../../src/store/id-sequence.js';

/**
 * schedule 域内存 fake（ARCH-UNIFY A4 自 inmemory-gov-store-schedule.ts mixin 独立成类）：
 * ScheduleRepository 全 12 条方法。方法体逐字搬迁、零行为变化。
 * demoSeed=true（默认）从 scheduleScenarioFixture seed 资源/占用窗口/交接线；false → 空数组（真实态空板）。
 */
export class InMemoryScheduleRepository implements ScheduleRepository {
  ...
  constructor(
    ...
    this.clock = clock;
    this.resources = demoSeed ? scheduleScenarioFixture.resources.map((r) => ({ ...r })) : [];
