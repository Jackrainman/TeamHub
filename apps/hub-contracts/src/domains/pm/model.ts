...
export const MemberObjectSchema = z.object({
  ...
  // AUTH-GATE（2026-09-04 用户拍板，撤销刀⑧②明文副本例外）：**绝不回存明文口令**——凭证只有
  // scrypt 散列一份。忘了就走 DELETE pin 重置（管理员/loopback）→ 首登强制重设流程。公网暴露下
  // 可恢复副本 = 库泄即全队口令泄，不再可接受。旧落盘里的 pinPlaintext 由服务端加载时剥离清扫。
...
/**
 * 读视图用成员（IDENTITY-LITE）：剥 `pinHash`（密钥纪律——凭证散列永不过读边界）。
