...
| DEPGRAPH-DRAG-CONNECT | pending (P1, D-052 立项) | code | **画布拖拽连线建依赖（Q3 拍板）** —— 用户要的「连线」是直接操作：xyflow `onConnect` handler，从节点 A 源 handle 拖到节点 B → 调 `createDependency(from=A,to=B)` → 成功后 invalidate dep-graph 自动重绘。节点开 connectable handles；自 edge / 重复边 / 成环（对照 AUDIT H1）拒绝并轻提示。语义保留：A 阻塞 B（不丢业务含义） |
