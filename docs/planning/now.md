...
current_task: "2026-07-11 **刀① BASELINE-CORE 已落地**（六步 S1–S6 + 收口，commit d3db6fe..e3a76a9，VERSION 0.10.3→0.11.5，minor）：Season 接线（GovernanceSnapshot.seasons + GET /api/seasons）/ baseline zod 独立域文件 / 独立 baselineStore（InMemory+File 两实现，独立落盘 baseline.json）/ GET·PATCH·POST-pass 三路由（过门 + I0 剥 passedBy 沿 confirmedBy 先例）/ deriveBaselineDrift 周粒度红黄绿 + 组归因 + 投资示警 / 总览 BaselineOverview『基准线 vs 实际』首屏 + 三版车模板 v1 seed + 投资三维录入。本机三包 verify:all 全绿（contracts 197 / server 232 / console 96）+ e2e-pillars 3 绿 + Playwright 首屏走查；偏离已回写 baseline-design.md §7『实现落点注记』。**下一步 = 刀② IDENTITY-LITE**（轻身份登录双模式：匿名整体保留 + 登录才写/个人视图、服务端注入身份、ownerId 选人、Member.pinHash）。// 上游文档：product-redefine-2026-07.md（DECIDED D-083）+ baseline-design.md（v1，§7 实现注记）；07-03 五件套终态 a64e7bc v0.10.3 详 completed-log；★本机有全套 node_modules+chromium 可本机验证；datalist 兼容性待真机确认。"
repo_sync: "2026-07-11：BASELINE-CORE 六步 + 收口（d3db6fe..e3a76a9 共 7 个 commit + 本 planning docs commit，VERSION 0.10.3→0.11.5，含 S1 一次 minor）已**本机全量验证**：三包 verify:all 全绿（hub-contracts 197 测 / hub-server 232 测含 e2e-pillars / hub-console 96 测，typecheck+build 均过）+ `test:local -- e2e-pillars` 3 测独立复跑绿 + Playwright 首屏走查基准线总览达成。本机 node24 全套 node_modules（WSL 免除，用户裁定；本轮全程无 WSL/ssh）。push 走已配 credential helper（共享库 ~/ruolin_huang/.gh-credentials，push 前 git fetch 防分叉）。 // 历史 2026-07-03：origin/master=c84f4a9（07-03 FF 合并 feat/plugin-core 六个模块化 commit：6fc32fb→c84f4a9，VERSION=0.9.7）。rainman WSL2 上次同步到 feat/plugin-core 同一 HEAD（07-02 验证全绿：三包 verify:all 432 测 + 迁移脚本 scripts/migrate-robottarget.mjs 真 gov.json 副本零丢失 + Playwright health-check 8 页 0 错等价 master + 真实未迁移 gov.json 新 schema 加载成功）；本轮 07-03 全部 commit 已经**本机全量验证**（WSL 跳过·用户裁定；本机 node24 首次装齐四包 node_modules + playwright chromium）：506 测全绿 + health-check 8 页 0 错 + 新端点冒烟全通。WSL 真机部署验证转为可选项。**部署前必做**：D-080 砍 `requires` 依赖类型 + FileGovStore fail-closed 校验后，旧 gov.json（含 `type:'requires'`）redeploy 直接起不来，须先跑 `scripts/migrate-robottarget.mjs` 或 reset `~/teamhub-data/gov.json`。 // 历史 2026-06-21：origin/master=9cd0b7a（D-078 图纸文件链路 + 表单修缮 + 部署/版本号修复），三包 verify:all 全绿（164/194/44）+ 真机 API 冒烟全过（上传/下载/重传清兄弟/落盘持久化/415·401·404·400）。 // 历史 2026-06-19：origin/master 曾推进到无人值守实现轮（5cb84cb：INV-BOM-CORE + PRESENCE-IMPL 安全增量 + WSL 验收截图 + lark bin 修，4 commit 已 push）。push 凭证走共享库 ~/ruolin_huang/.gh-credentials，按 D-064 默认 commit+push（push 前 git fetch 防分叉）。rainman WSL2 真机已同步到 736f0c3 并起服 4177 跑通 Playwright 验收（loopback，TEAMHUB_INV_DATA_FILE=~/teamhub-data/inventory.json）；注意 WSL 在 ssh 会话关闭后会 reap 所有进程（含 tmux），常驻部署须保持连接或 enable-linger，验收已在单会话内完成。bundle 传 100.78.202.84 实测 ssh BatchMode 可达、字节比对无截断。详见 memory teamhub-remote-access。"
frontier:                                # 本轮(07-11)=产品重定义文档轮完成；下一批按路线 v4（D-083）：基准线→轻身份→我的视图→store 拆分+SQLite
  ...
  - IDENTITY-LITE                         # 刀②轻身份登录双模式：匿名模式整体保留供选择；身份模式=匿名可读一切+登录才写/个人视图(选人+可选PIN)；服务端注入身份替代客户端自报(≥6 写路由+5 Draft 类型)；ownerId 自由文本→选人；Member 加 optional pinHash
  - MY-VIEW                               # 刀③：我负责的任务∩未被依赖卡住的；blockedBy 派生已有，等 IDENTITY-LITE 落地即白捡
  - STORE-SPLIT-SQLITE                    # 刀④：先按 pm-core/schedule 边界拆 GovStore god-interface(21方法/6域)，再分模块增量迁 SQLite；8 个排班方法先补落盘设计；id/clamp 抽独立纯函数三实现共享
  - LEARN-DIRECTION-REDESIGN              # 缺人方向页→「学习方向」：跨工种学习地图(设计稿 §5 初稿)×队内缺口，只对本人、只建议不指派；兴趣声明暂不建(D-083)
  - AUDIT-DEBT-2026-07                    # 审计债(设计稿 §9-④)：GroupKind 闭集放宽/convergenceScope 收口/pm-requests 解绑 robotics 词汇/main.ts tenantConfig 接线/工具条特例——可与上面穿插
