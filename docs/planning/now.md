...
frontier:                                # PRESENCE-RECONCILE-LOCK 需定稿；其余仍卡外部基建 / 用户排期
  - IA-REFACTOR                          # 【2026-06-20 用户加·待讨论后做·**单开 branch**·先不讨论】信息架构重构：10 平铺页按数据域重组。渐进式 4 阶段(计划见 docs/design/sched-date-relay-robot-redesign.md B 节)：①机器人队页(机器人管理+在场排班合一，收益最高，A1/A2/A3 是其地基)→②项目页(看板+依赖图视图切换，单一录入/改状态入口)→③知识页(知识库+图纸档案)→④导航分组+工作台落地页。用户明确：讲完单开 branch 做，不在 master 直改。
  - SCHEDULE-DESIGN-LOCK                  # 【2026-06-20 用户加·待定·先不讨论】在场排班整体设计方案未拍：用户反映「现在什么都没有、也没办法加」——空态/录入动线/默认数据(日期模型下一天一计划如何起步)/「没法加」的实际用法需先出设计稿(仿 inv-bom-core.md)再继续做。零件已就位(A1 日期选择器 + A2 +加一棒/删一棒 + RelayCanvas 拖拽/ETA/交接线)，但整页"怎么用、空了怎么起头"的设计未定。
