...
有人卡住又不好意思开口时，让卡点在依赖图上自己显出来，系统替他说一声。不盯个人。**
...
Teamhub 是机器人战队的协作中枢：CASE 工具、团队交流中心、战队数据库三合一，面向 5–15 人小作坊，没有硬截止。真相放在系统关系库里，飞书只管汇报、通知和一键 check-in，两边不双写。还有一条底线（D-037）：人手填的信号只回给本人当帮助；对第三方只暴露结构键（任务、组、资源），不暴露人。公平靠给被卡的人正名，不靠盯梢。
...
Skill 接受碎片化的输入，再按协议把它整理成结构。`debug-checklist` 给一句话症状就能跑。

AI 不是只来润色文字的。它参与症状解析、生成检查项、追记排查过程、把结果织进归档，每一步都有 Skill 协议兜着。底层是 SKILL.md 协议加镜像同步 hook，`.agents/` 三方共用（Claude Code / OpenCode / Codex），所以 AI 的行为是可控、可追踪、可复用的。

解决过的问题都落成 `.debug-archive/` 里的 markdown。下次撞上同类症状，Trail 会把相关历史翻出来对照。

录入这件事没有强制：飞书消息进来，走 Skill，出 markdown，进 git，没有"请打开网页填个表单"这一步。飞书已经有的（消息、通知、多维表格）不抢，只补它缺的那块：调试专用结构、症状关联、赛季年鉴。
...
`apps/hub-*` 是当前在做的协作中枢（hub-server / hub-contracts / hub-console）。根目录有个一键启动脚本：先把前端构建成静态站，再由 hub-server 单端口（默认 4177）同时托管 console 和 API。
...
知识库语料默认落在 `~/teamhub-data/kb.json`，重启不丢、closeout 会累积回灌；要换路径设 `TEAMHUB_KB_DATA_FILE`。一个提醒：写端点现在还没鉴权（见 `docs/planning/code-audit-2026-06-14.md` 的 H3），暂时只在可信内网里开。
...
Teamhub 是给机器人战队这种 5–15 人小作坊用的调试知识枢纽，不打算做成万能 AI 助手：

- 问题突然冒出来时，Skill 给个检查方向
- 队友卡住时，Bridge 让阻塞被看见、被配对
- 赛季结束时，Trail 把这一年的排查轨迹拼成能读的东西
- 新人进来时，能翻历史看前人是怎么趟过来的

说白了，就是想让现场每一次救火都攒下点能复用的经验。
...
数据落在 markdown，权威源是 git，飞书当数据层，AI 把口头经验翻译成结构
