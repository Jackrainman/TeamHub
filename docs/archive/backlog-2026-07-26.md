...
| HUB-HERMES-ADAPTER | **TeamHub 侧 done(v0.43.4)** / 飞书侧 pending | code/design | **统一触点能力：项目调用 Hermes/openclaw**。TeamHub 入站 API 已落地：`POST /api/hermes/inbound`（inv-query 按名称/类别/机器人查 + inv-record 加/减/调拨，source=hermes，原始文本规则匹配+结构化命令双收）。**剩余**：飞书侧 skill 编写（用户侧）+ 鉴权最终方案（见 HERMES-AUTH-SCHEME）+ .env token 管理设计化（见 ENV-TOKEN-DESIGN）|
| HERMES-AUTH-SCHEME | pending (decision-needed) | design | **Hermes 鉴权最终方案**：当前暂用 WRITE_TOKEN Bearer；用户期望"点链接授权"体验（飞书 app 授权流 / 专用 API Key / 绑定队员会话）。调研飞书开放平台 app 创建/授权流，写成方案供用户拍板。|
| ENV-TOKEN-DESIGN | pending (decision-needed) | design | **.env token 管理设计化**：用户不希望手写 .env，期望通过 UI/流程设计管理 WRITE_TOKEN 等敏感配置（如向导中生成、设置页轮换）。待讨论具体方案。|
| CSV-TEMPLATE-VALIDATION | pending | code/design | **CSV 模板数据验证（下拉提示）**：所有 CSV 导入（名册/车队/库存）的模板加 Excel 数据验证下拉（年级/编号/状态等固定选项），或改 xlsx 模板。痛点=用户每次手填不一定对。当前预览表已有行内编辑兜底，此为模板侧增强。|
...
| WIZARD-SEASON-STEP | done(2026-07-25, v0.42.0) | code | 向导赛季步④（纯 console）：suggestSeason 预填赛季名/学期开始 + 比赛日选填 → createSeason（startsAt=学期开始→ISO）+ 两锚点齐则 generateRoboconBaselineTemplate+updateBaseline 落基准线模板；已有 active 赛季直接下一步；可跳过（刀⑨ app 内空态兜底） |
